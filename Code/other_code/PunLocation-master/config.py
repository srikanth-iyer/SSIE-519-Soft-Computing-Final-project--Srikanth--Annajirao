...
class Config():
    def __init__(self, task, batch_size, epoch):
        ...
        # self.emb_file = os.path.join('..', 'GloVe', 'glove.6B', 'glove.6B.100d.txt') if not self.debug else os.path.join('..', 'GloVe', 'glove.6B', 'glove.6B.100d.txt.orig')  # path to pre-trained word embeddings

        self.emb_file = r'..\..\GloVe\glove.6B\glove.6B.100d.txt' if not self.debug else r'..\..\GloVe\glove.6B\glove.6B.100d.txt.orig'  # path to pre-trained word embeddings
